...
for i in $(seq $start $end); do
    ...
    mnemonic=$(yq e ".[$i].mnemonic" "$BASEDIR/artifacts/mnemonics$minimal_suffix.yaml")
    count=$(yq e ".[$i].count" "$BASEDIR/artifacts/mnemonics$minimal_suffix.yaml")
    ...
    bazel run $minimal//cmd/validator:validator wallet recover -- --wallet-dir=$BASEDIR/validator-$i --mnemonic-file=$BASEDIR/validator-$i/mnemonic.txt --mnemonic-25th-word-file=$BASEDIR/artifacts/wallet/password.txt --num-accounts=$count --wallet-password-file=$BASEDIR/artifacts/wallet/password.txt --accept-terms-of-use
